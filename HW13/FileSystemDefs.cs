...
namespace CS422
{
	...
	public class StdFSFile : File422
	{
		...
		public override Stream OpenReadOnly()
		{
			try
			{
				return File.OpenRead(FullPath.Make(Parent, name));
			}
			catch (Exception)
			{
				return null;
	...
	public class StdFSDir : Dir422
	{
		...
		public override bool ContainsFile(string name, bool recursive)
		{
			...
			// Recursive
			if (File.Exists(FullPath.Make(this, name)))
			{
				return true;
			}
		...
		public override bool ContainsDir(string name, bool recursive)
		{
			...
			// Recursive
			if (Directory.Exists(FullPath.Make(this, name)))
			{
				return true;
			}
		...
		{
			string fullPath = FullPath.Make(this, name);

			if (!Validity(name) || File.Exists(fullPath))
			{
				return null;
			}
		...
		public override File422 CreateFile(string name)
		{
			...
			if (!Validity(name) || Directory.Exists(fullPath))
			{
				return null;
			}
		...
		public override Dir422 GetDir(string name)
		{
			...
			return (Directory.Exists(FullPath.Make(this, name))) ?
				new StdFSDir(name, this) : null;
		}
		...
		public override IList<Dir422> GetDirs()
		{
			...
			foreach (var dir in Directory.GetDirectories(FullPath.Make(this)))
			{
				dirs.Add(new StdFSDir(dir.Substring(dir.LastIndexOf('/') + 1), this));
			}
		...
		public override IList<File422> GetFiles()
		{
			...
			foreach (var file in Directory.GetFiles(FullPath.Make(this)))
			{
				files.Add(new StdFSFile(file.Substring(file.LastIndexOf('/') + 1), this));
			}
